    ...
    public List<Blog> getAllBlog() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                boolean status = rs.getBoolean("BlogStatus");
    ...
    public ArrayList<Blog> getBlogFromStatus(int status, int uid) throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Tạo đối tượng Blog từ dữ liệu từ cơ sở dữ liệu
                Blog blog = new Blog(blogId, blogTitle, blogImage, blogDetail, postDate, blogTagId, true, uid);
                blogS.add(blog);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public List<Blog> getBlogFromTag(int tagid) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int uid = rs.getInt("UserID");
                // Tạo đối tượng Blog từ dữ liệu từ cơ sở dữ liệu
                Blog blog = new Blog(blogId, blogTitle, blogImage, blogDetail, postDate, tagid, true, uid);
                blogS.add(blog);
            }
        } catch (Exception e) {
            e.printStackTrace();
